...
\noindent The Home Page's function is to summarize the useful data that the user can later analyze more in detail in other sections. Specifically, there is a greeting message and two boxes that display: \begin{itemize}
...
\vspace{0.5cm}
\noindent{\scshape{Subscription List Page}}
\vspace{0.2cm}

\noindent The Subscription List Page shows two separate lists of plans: \begin{itemize}
...
\vspace{0.5cm}
\noindent{\scshape{Statistics Page}}
\vspace{0.2cm}

\noindent The Statistics Page renders some graphs, in order to visualize the aggregated data about the memberships. The plans can be flagged with only one of the following categories:
\begin{itemize}
    \item[--] Movies \& TV,
    \item[--] Shopping,
    \item[--] Music,
...
\subsection{Firebase}
gestire automaticam auth: manages registration e invio mail recupero, verifica mail

easy to one endpoint (interface)

verifica input messa da noi client oltre che firebase auth (mail e pwd)

quando ti registri hai anche nome in authentication
utilizzato per amici (riconoscerli facilmente) no username (no univoco), uid per user e' mail

\subsection{Cloud Functions}
cloud functions = node.js backend logic ci sono librerie di firebase che ti permettono di accedere a firestore db dalle cloud functions
https requests, firebase verifica che siano davvero generate da application
controllano che chi chiama funz abbia permessi necessari
solo owner puo modificare subs
possibile implementare trigger per db per mantenere stato consistente
messaggi di errore specifici come risposte
utili per debug e per visualizzazione frontend

\subsection{Firestore database}
collection of documents
dati sono semplici nop complex structure, this solution allows for a more agile solution

firestore db settato per essere inaccessibile da esterno
solo admin puo effettuare modifiche - protezione db da richieste esterne
tutte funz di manipolaz dati salvaraggio e data retrieval tramite cloud functions

\subsection{Redux Toolkit}
redux toolkit latest version of redux
managed workflow (vedi sito)
redux has state aggiornato ogni cambio pagina in background, cosi user experience ottimale
centralizzata, permette di avere stato coerente in varie schermate di application
